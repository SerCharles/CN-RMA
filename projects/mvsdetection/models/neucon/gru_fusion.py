...
class GRUFusion(nn.Module):
    ...
    def __init__(self, thresholds, n_layers, voxel_size, n_voxels, ch_in=None, direct_substitute=False, use_gru_fusion=True):
        ...
        if direct_substitute:
            ...
            for i, ch in enumerate(ch_in):
                conv_gru_dict = dict(type='ConvGRU',
                                     hidden_dim=ch,
                                     input_dim=ch,
                                     pres=1,
                                     vres=self.voxel_size * 2 ** (self.n_scales - i))
                conv_gru = build_neck(conv_gru_dict)
                self.fusion_nets.append(conv_gru)
    ...
    def convert2dense(self, current_coords, current_values, coords_target_global, tsdf_target, relative_origin,
        ...
        dim = (torch.Tensor(self.n_voxels).cuda() // 2 ** (self.n_layers - scale - 1)).int()
        ...
        if self.use_gru_fusion is False:
        ...
        if self.use_gru_fusion is True:
    ...
    def save_mesh(self, scale, outputs, scene):
        ...
        outputs['origin'].append(min_c * self.voxel_size * (2 ** (self.n_layers - scale - 1)))
    ...
    def forward(self, coords, values_in, inputs, scale=2, outputs=None, save_mesh=False):
        ...
        interval = 2 ** (self.n_layers - scale - 1)
        ...
        for i in range(batch_size):
            ...
            if 'occ_list' in inputs.keys():
                # get partial gt
                occ_target = inputs['occ_list'][self.n_layers - scale - 1][i]
                tsdf_target = inputs['tsdf_list'][self.n_layers - scale - 1][i][occ_target]
