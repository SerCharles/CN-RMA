...
class CollectData(object):
    def __call__(self, data):
        ...
        result['world_to_aligned_camera'] = DC(data['world_to_aligned_camera'])
        result['tsdf_list'] = DC(data['tsdf_list'])
        result['occ_list'] = DC(data['occ_list'])
        result['proj_matrices'] = DC(data['proj_matrices'])
        result['vol_origin'] = DC(data['vol_origin'])
        result['vol_origin_partial'] = DC(data['vol_origin_partial'])
        result['scene'] = DC(data['scene'], cpu_only=True)
        result['fragment'] = DC(data['fragment'], cpu_only=True)
...
class RandomTransformSpace(object):
    ...
    def __init__(self, voxel_dim, voxel_size, random_rotation=True, random_translation=True,
                 paddingXY=1.5, paddingZ=.25, origin=[0, 0, 0], max_depth=3.0):
        """
        Args:
        ...
        self.random_r = torch.rand(1)
        self.random_t = torch.rand((1, 3))
