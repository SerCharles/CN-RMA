...
"""ScanNet Dataset used in pretraining FCAF3D
"""
...
class ScanNetDataset(Custom3DDataset):
    ...
    def show(self, results, out_dir, show=True, pipeline=None):
        ...
        for i, result in enumerate(results):
            name = self.data_infos[i]['point_cloud']['lidar_idx']
            if not os.path.exists(os.path.join(out_dir, name)):
                os.makedirs(os.path.join(out_dir, name))
            file_name = os.path.join(out_dir, name, name + '_fcaf3d_retrain.npz')
            boxes = result['boxes_3d'].tensor.numpy()
            boxes[..., 2] += boxes[..., 5] / 2
            scores = result['scores_3d'].numpy()
            labels = result['labels_3d'].numpy()
            np.savez(file_name, boxes=boxes, scores=scores, labels=labels)
