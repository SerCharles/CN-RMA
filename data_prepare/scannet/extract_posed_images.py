...
def process_scene(source_path, target_path, limit, idx):
    """Process single ScanNet scene.

    Extract RGB images, poses and camera intrinsics.
    """
    data = SensorData(os.path.join(source_path, idx, f'{idx}.sens'), limit)
    output_path = os.path.join(target_path, idx)
...
def process_directory(source_path, target_path, limit, nproc):
    print(f'processing {source_path}')
    mmcv.track_parallel_progress(
        func=partial(process_scene, source_path, target_path, limit),
        tasks=os.listdir(source_path),
        nproc=nproc)


if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument('--max-images-per-scene', type=int, default=-1)
    parser.add_argument('--nproc', type=int, default=8)
    parser.add_argument('--data_root', type=str, default='/home/sgl/MVSDetection/data/scannet/')
    parser.add_argument('--save_path', type=str, default='/home/sgl/MVSDetection/data/scannet/posed_images')
    ...
    source_path = os.path.join(args.data_root, 'scans')
    if os.path.exists(source_path):
        process_directory(source_path, args.save_path, args.max_images_per_scene, args.nproc)
    # process test scenes
    source_path = os.path.join(args.data_root, 'scans_test')
    if os.path.exists(source_path):
        process_directory(source_path, args.save_path, args.max_images_per_scene, args.nproc)
