...
import json
model_atlas = torch.load('/data/shenguanlin/work_dirs_atlas/final.ckpt')['state_dict']
model_ray = torch.load('/data1/shenguanlin/ray_marching/epoch_12.pth')['state_dict']
model_mine = torch.load('/data/shenguanlin/work_dirs_atlas/pipeline_link.pth')['state_dict']
...
for key in model_ray.keys():
    atlas[key] = model_ray[key].shape 
    if 'backbone' in key:
        fcaf3d_backbone[key] = model_ray[key].shape 
    elif 'neck_with_head' in key:
        fcaf3d_head[key] = model_ray[key].shape
...
model_mine_full = torch.load('/data/shenguanlin/work_dirs_atlas/pipeline_link.pth')
model_mine_full['state_dict'] = new_state_dict
torch.save(model_mine_full, '/data/shenguanlin/work_dirs_atlas/ray_marching_base.pth')
